...
export default function JobTabs({activeTab, setActiveTab}) {
  return (
    <View style={styles.container}>
      <FlatList data={tabs} horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{columnGap:SIZES.small /2}} keyExtractor={(item)=>item} renderItem={({item})=>(
        <TouchableOpacity style={styles.tabBtn(activeTab, item)} onPress={()=>setActiveTab(item)}>
